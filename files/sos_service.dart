...
class SosService {
  ...
  Future<SosTriggerResult> triggerSos() async {
    ...
    if (uid == null) return SosTriggerResult.notAuthenticated;

    final pos = await LocationService().getCurrentPosition();

    _lastSosTrigger = DateTime.now();

    // Call Cloud Function — it handles geo-query + FCM dispatch
    try {
      ...
      final sosId = result.data['sos_id'] as String;
  ...
  // ─── Cancel SOS (within 5-second window) ─────────────────────────────────
  ...
  Stream<SosEvent?> watchActiveSos() {
    ...
    return _firestore
        ...
        .limit(1)
        .snapshots()
        .map((snap) => snap.docs.isEmpty ? null : SosEvent.fromDoc(snap.docs.first));
  }

  // ─── Watch incoming SOS alerts (for responder) ────────────────────────────

  Stream<List<SosEvent>> watchIncomingSos() {
    return _firestore
        .collection('sos_events')
        .where('status', isEqualTo: 'ACTIVE')
        .orderBy('timestamp', descending: true)
...
class SosTriggerResult {
  ...
  static const SosTriggerResult notAuthenticated =
